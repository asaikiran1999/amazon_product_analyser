	...
	if t==1:
		...
		if st.button('analyse'):
			...
			st.text('labeling good and bad started')      
                for i in reviews_list:if 'img' not in i and 'video' not in i:sentiment = [] for i in range(len(reviews_list)):sentiment.append(int(model.predict(vec.transform([reviews_list[i]]))))
